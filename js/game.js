// Plinkoo Game (import-map friendly): calm physics, instant spawn, neon post-processing
...
// Postprocessing via import map (examples/)
...
(() => {
  ...
  function showSettings(){ gsap.to(settingsPanel, { x:0, duration:.35, ease:'expo.out' }); }
  function hideSettings(){ gsap.to(settingsPanel, { x:'110%', duration:.35, ease:'expo.in' }); }
  ...
  function layoutOverlays() {
    ...
    Object.assign(boardFrame.style,{
      ...
      width:frame.width+'px', height:frame.height+'px', display:'block'
    });
    Object.assign(slotTray.style,{
      left:tray.x+'px', top:tray.top+'px',
      width:tray.width+'px', height:tray.height+'px', display:'block'
    });
    Object.assign(boardDivider.style,{
      left:frame.x+'px', width:frame.width+'px',
      top:(pTrayTopLeft.y - 1)+'px', display:'block'
    });
  ...
  async function spawnBall({ username, avatarUrl }){
    ...
    for(let i=0;i<multi;i++) spawn({ username, avatarUrl });
  }
